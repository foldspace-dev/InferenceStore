...
mapping, capability reporting, streaming-event mapping, the local privacy
boundary, and threading contract — is real and fully unit-tested with a fake
runtime, and the heavyweight native binding is a clean integration point.
...
+ logic are complete and tested; wire a real `LiteRtLmRuntime` to run inference.
